...
class AttentionModel(nn.Module):
    ...
    def forward(self, x, x1, x2):
        ...
        anc = self.output_layer(concatenated_tensor)

        #________________________________________________________________
        #Reverse the x
        reversed_x1 = torch.flip(x1, dims=[1])

        # Pass input through LSTM layers
        output1, (h_n1, c_n1) = self.lstm(x1)
        reversed_output1, (reversed_h_n1, reversed_c_n1) = self.lstm(reversed_x1)
        
        # Compute attention weights
        alpha1 = F.softmax(self.attention_layer(output1), dim=1)
        reversed_alpha1 = F.softmax(self.attention_layer(reversed_output1), dim=1)
        
        # Compute attention output
        attention_output1 = torch.sum(alpha1 * output1, axis=1)
        reversed_attention_output1 = torch.sum(reversed_alpha1 * reversed_output1, axis=1)

        # Concatenate them
        concatenated_tensor1 = torch.cat([attention_output1, reversed_attention_output1], dim=1)

        # Pass attention output through output layer
        one = self.output_layer(concatenated_tensor1)

        #________________________________________________________________
        #Reverse the x
        reversed_x2 = torch.flip(x2, dims=[1])

        # Pass input through LSTM layers
        output2, (h_n2, c_n2) = self.lstm(x2)
        reversed_output2, (reversed_h_n2, reversed_c_n2) = self.lstm(reversed_x2)
        
        # Compute attention weights
        alpha2 = F.softmax(self.attention_layer(output2), dim=1)
        reversed_alpha2 = F.softmax(self.attention_layer(reversed_output2), dim=1)
        
        # Compute attention output
        attention_output2 = torch.sum(alpha2 * output2, axis=1)
        reversed_attention_output2 = torch.sum(reversed_alpha2 * reversed_output2, axis=1)

        # Concatenate them
        concatenated_tensor2 = torch.cat([attention_output2, reversed_attention_output2], dim=1)

        # Pass attention output through output layer
        two = self.output_layer(concatenated_tensor2)
...
if channel == 3:
    print(f'BiLstm WISDM Triplet accuracy: {accuracy}')  
else:
    print(f'BiLstm META_HAR Triplet accuracy: {accuracy}')  
# print(f'F1 score: {f1}') 
