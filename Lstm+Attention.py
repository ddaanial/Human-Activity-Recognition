...
for epoch in range(num_epochs):
    ...
    if channel == 3:
        with open('Results/LSTM+ATTENTION/WISDM/WISDM_Loss.txt', 'a') as file:
            # Write the accuracy 
            file.write(f'Epoch [{epoch+1}/{num_epochs}], loss: {epoch_loss:.4f}, accuracy: {epoch_acc:.2%}\n')
    else:
        with open('Results/LSTM+ATTENTION/Meta_Har/Meta_Har_Loss.txt', 'a') as file:
            # Write the accuracy 
            file.write(f'Epoch [{epoch+1}/{num_epochs}], loss: {epoch_loss:.4f}, accuracy: {epoch_acc:.2%}\n')

# Save the best model state to a file
if channel == 3:
    torch.save(best_model_state, 'Results/LSTM+ATTENTION/WISDM/WISDM_best_model.pth')
else:
    torch.save(best_model_state, 'Results/LSTM+ATTENTION/Meta_Har/Meta_Har_best_model.pth')
...
with torch.no_grad():
    ...
    if channel == 3:
        with open('Results/LSTM+ATTENTION/WISDM/WISDM_Result.txt', 'w') as file:
            # Write the accuracy and loss to the file
            file.write(f'test loss: {test_loss}\n')
            file.write(f'test accuracy: {test_acc}\n')
            file.write(f'F1 Score: {f1}\n')
    else:
        with open('Results/LSTM+ATTENTION/Meta_Har/Meta_Har_Result.txt', 'w') as file:
            # Write the accuracy and loss to the file
            file.write(f'test loss: {test_loss}\n')
            file.write(f'test accuracy: {test_acc}\n')
            file.write(f'F1 Score: {f1}\n')
...
if channel == 3:
    plt.savefig('Results/LSTM+ATTENTION/WISDM/WISDM_confusion_matrix.png')
else:
    plt.savefig('Results/LSTM+ATTENTION/Meta_Har/Meta_Har_confusion_matrix.png')
